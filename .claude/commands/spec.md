...
1. Use the Fima MCP tools to locate the component, layer or frame.
2. Extyract only information that is useful for implementation, such as: 
   - Dimensions and layout (grid, spacing, alignment)
   - Key typography tokens (font, family, size wieght)
...
4. If lookup fails or the tools are not available, record a note like: 
   - `"Design reference could not be retrieve. See Figma manually for details."`
